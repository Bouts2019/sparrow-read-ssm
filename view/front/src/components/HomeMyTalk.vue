...
export default {
  ...
  methods: {
    ...
    findMyTalks() {
      this.$http.get('http://localhost:8080/sparrow/talk/user/' + this.myId).then(res => {
        this.talks = res.data.data
        this.talks.sort((a, b) => b.publishTime - a.publishTime)
